...
import { DEFAULT_DEBATE_MODE } from "@/lib/debate-mode";
...
function initializeDatabase(database: Database.Database) {
  if (!global.__multiAgentDbInitialized) {
    database.exec(`
      CREATE TABLE IF NOT EXISTS runs (
        ...
        current_milestone_turn INTEGER NOT NULL DEFAULT 0,
      ...
      CREATE TABLE IF NOT EXISTS referee_decisions (
        ...
        carry_forward_notes_json TEXT,
        diminishing_returns_json TEXT,
  ...
  ensureColumn(database, "runs", "debate_mode", `TEXT DEFAULT '${DEFAULT_DEBATE_MODE}'`);
  ensureColumn(database, "runs", "current_milestone_turn", "INTEGER NOT NULL DEFAULT 0");
  ensureColumn(database, "runs", "current_task_index", "INTEGER NOT NULL DEFAULT 0");
  ensureColumn(database, "referee_decisions", "blocking_issues_json", "TEXT");
  ensureColumn(database, "referee_decisions", "carry_forward_notes_json", "TEXT");
  ensureColumn(database, "referee_decisions", "diminishing_returns_json", "TEXT");
}
